...
class Qshot_settings(QMainWindow):
    ...
    def block1(self):
        ...
        self.preview_checkbox.setText("prew")
        self.preview_checkbox.setObjectName("opacity_prew_checkbox")
        self.preview_checkbox.clicked.connect(self.on_click)
        ...
        hbox3.addWidget(self.preview_checkbox)
    ...
    def block2(self):
        ...
        self.block2_group_box.setToolTip(self.hotkey_tooltip)
    ...
    def update_opacity(self, value):
        """updates opacity value on frame and if prew checkbox checked also emits signal to main frame to update opacity"""
    ...
    def on_click(self):
        ...
        if(sender.objectName() == "background_color_button"):
            ...
            if(write_status):
                self.show_success_popup("Settings reset to default, please restart the Qshot for changes to be effective")
                self.read_cfg_file()
                self.variable_to_ui()
                if(self.preview_checkbox.isChecked()):
            ...
            if(variable_status):
                ...
                if(write_status):
                    self.show_success_popup("Settings saved, please restart the Qshot for changes to be effective")


        # checkboxes
        elif(sender.objectName() == "opacity_prew_checkbox"):
            if(self.preview_checkbox.isChecked()):
                self.activate_preview()
            else:
                self.revert_preview()
...
def start_settings_with_event_loop(cfg_path, icon_path):
    """if the main window can not open because of the cfg file, settings needs its own event loop to stay open"""
